...
public class Picture extends SimplePicture 
{
  ...
  public Picture flipHorizontalSquare(int x, int y, int size){
    ...
    if (x>=this.getWidth()&&y-size+1>=this.getHeight()){
      System.out.println("Sorry you the square flipped is not valid");
      return result;
    }
    else if (x+size-1>=this.getWidth()&&y>=this.getHeight()){
      endPointX = this.getWidth()-1;
      endPointY = this.getHeight()-1;
    }
    else if (x+size-1>=this.getWidth()){
      endPointX = this.getWidth()-1;
    }
    else if (y>=this.getWidth()){
      endPointY = this.getHeight()-1;
    }
    else{
      startPointX = x;
      startPointY = y-size+1;
      endPointX = x+size/2;
      endPointY = y;
