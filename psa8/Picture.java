...
public class Picture extends SimplePicture 
{
  ...
  public void alphaBlending(int x, int y, Picture background)
  {
    ...
    int startSourcex = 0;
    int startSourcey =0;
    int endPointx = x+this.getWidth()-1;
    int endPointy = y+this.getHeight()-1;
    Pixel targetPixel=null;
    Pixel sourcePixel=null;
    if (x>endPointx||y>endPointy||y+this.getHeight()-1<0||x+this.getWidth()-1<0){
      return;
    }
    if (x<0){
      startPointx=0;
    ...
    if (endPointx>background.getWidth()-1){
      endPointx = background.getWidth()-1;
    }
    if(y<0){
      startPointy=0;
      startSourcey=-y;
    }
    if (endPointy>background.getHeight()-1){
      endPointy = background.getHeight()-1;
    }
    
    //loop through the the specifed region of background and the source photo to blend two together. 
    for(SourceX =startSourcex,targetX=startPointx;targetX<=endPointx;targetX++,SourceX++){
      for(SourceY=startSourcey,targetY=startPointy;targetY<=endPointy;targetY++,SourceY++){
        targetPixel = background.getPixel(targetX,targetY);
        sourcePixel = this.getPixel(SourceX,SourceY);
        alpha = sourcePixel.getAlpha();
        targetPixel.setRed((int)(alpha / 255 * sourcePixel.getRed() + (1 - alpha / 255) * targetPixel.getRed()));
        targetPixel.setBlue((int)(alpha / 255 * sourcePixel.getBlue() + (1 - alpha / 255) * targetPixel.getBlue()));
        targetPixel.setGreen((int)(alpha / 255 * sourcePixel.getGreen() + (1 - alpha / 255) * targetPixel.getGreen()));
      }
    }
  }
